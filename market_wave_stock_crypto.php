...
$asset_type = isset($_GET['asset_type'])
    ? strtolower(trim((string) $_GET['asset_type']))
    : 'stock';

if (!in_array($asset_type, ['stock', 'crypto'], true)) {
    $asset_type = 'stock';
}

$raw_symbol = isset($_GET['symbol'])
    ? strtoupper(trim((string) $_GET['symbol']))
    : ($asset_type === 'crypto' ? 'BTC' : 'TSLA');

$raw_symbol = preg_replace('/\s+/', '', $raw_symbol);

if (!preg_match('/^[A-Z0-9.\-^]{1,15}$/', $raw_symbol)) {
    $raw_symbol = $asset_type === 'crypto' ? 'BTC' : 'TSLA';
}

if ($asset_type === 'crypto') {
    $ticker = str_ends_with($raw_symbol, '-USD')
        ? $raw_symbol
        : $raw_symbol . '-USD';
} else {
    $ticker = $raw_symbol;
...
function fetchYahooChartCsv(string $ticker): array
{
    ...
    $url = "https://query1.finance.yahoo.com/v8/finance/chart/{$encoded}"
        . "?range=5d&interval=5m&includePrePost=true&events=div%2Csplits&_=" . time();
...
$cache_seconds = 0; // fetch Yahoo on every page request
...
<head>
    ...
    <title><?= htmlspecialchars($ticker) ?> <?= $asset_type === 'crypto' ? 'Crypto' : 'Stock' ?> Wave Dashboard</title>
    <style>
        :root {
            color-scheme: dark;
            --bg: #07111f;
...
<main class="shell">
    <header class="hero">
        <div>
            ...
            <p class="subtitle" style="margin-top:10px;color:var(--accent);font-weight:800;"><?= $asset_type === 'crypto' ? '24/7 cryptocurrency market' : 'U.S. stock market' ?></p>
            <p class="subtitle">
                Switch between stocks and 24/7 cryptocurrency markets, then run the same directional,
                differential, integral, and projected wave analysis on fresh Yahoo Finance data.
            </p>
        </div>
    ...
    <form class="control-panel" method="get" action="" id="marketForm">
        <div class="field">
            <label>Market type</label>
            <div class="asset-switch" role="radiogroup" aria-label="Market type">
                <label class="asset-option">
                    <input type="radio" name="asset_type" value="stock" <?= $asset_type === 'stock' ? 'checked' : '' ?>>
                    <span>Stock</span>
                </label>
                <label class="asset-option">
                    <input type="radio" name="asset_type" value="crypto" <?= $asset_type === 'crypto' ? 'checked' : '' ?>>
                    <span>Crypto</span>
                </label>
            </div>
        </div>
        ...
        <div class="field">
            <label for="symbol" id="symbolLabel"><?= $asset_type === 'crypto' ? 'Crypto symbol' : 'Stock ticker' ?></label>
            <div class="input-wrap">
                <span class="input-prefix" id="symbolPrefix"><?= $asset_type === 'crypto' ? '₿' : '$' ?></span>
                <input
                    id="symbol"
                    name="symbol"
                    type="text"
                    value="<?= htmlspecialchars($asset_type === 'crypto' ? preg_replace('/-USD$/', '', $ticker) : $ticker) ?>"
                    maxlength="15"
                    autocomplete="off"
                    spellcheck="false"
                    placeholder="<?= $asset_type === 'crypto' ? 'BTC' : 'TSLA' ?>"
                >
            </div>
        </div>

        <button type="submit">Load live market</button>
    </form>
    ...
    <section class="metrics" aria-label="Analysis summary">
        <article class="metric">
            <div class="metric-label"><?= $asset_type === 'crypto' ? 'Selected crypto' : 'Selected stock' ?></div>
            <div class="metric-value"><?= htmlspecialchars($ticker) ?></div>
            <div class="metric-note"><?= $asset_type === 'crypto' ? 'Trades continuously, 24/7' : 'Regular and extended-hours data' ?></div>
        </article>
        ...
        <article class="metric">
            ...
            <div class="metric-value">5-minute</div>
            <div class="metric-note">Yahoo Finance · five-day window</div>
        </article>
    </section>
    ...
    <p class="footer-note">
        ...
        Stocks can pause outside supported trading sessions. Crypto symbols such as BTC, ETH, SOL,
        XRP, and DOGE continue receiving later numbers around the clock.
    </p>
</main>
<script>
const assetRadios = document.querySelectorAll('input[name="asset_type"]');
const symbolInput = document.getElementById('symbol');
const symbolLabel = document.getElementById('symbolLabel');
const symbolPrefix = document.getElementById('symbolPrefix');

assetRadios.forEach((radio) => {
    radio.addEventListener('change', () => {
        const isCrypto = radio.value === 'crypto' && radio.checked;

        if (!radio.checked) return;

        symbolLabel.textContent = isCrypto ? 'Crypto symbol' : 'Stock ticker';
        symbolPrefix.textContent = isCrypto ? '₿' : '$';
        symbolInput.placeholder = isCrypto ? 'BTC' : 'TSLA';

        const current = symbolInput.value.trim().toUpperCase();

        if (isCrypto) {
            if (!current || ['TSLA', 'AAPL', 'MSFT', 'NVDA', 'SPY'].includes(current)) {
                symbolInput.value = 'BTC';
            } else {
                symbolInput.value = current.replace(/-USD$/, '');
            }
        } else {
            if (!current || ['BTC', 'ETH', 'SOL', 'XRP', 'DOGE'].includes(current.replace(/-USD$/, ''))) {
...
setTimeout(() => {
    ...
    url.searchParams.set('asset_type', '<?= htmlspecialchars($asset_type, ENT_QUOTES) ?>');
