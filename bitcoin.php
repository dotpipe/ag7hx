...
<?php
// ================================================
// Bitcoin Wave Example for NASDAQ Tickers (Yahoo Finance)
// ================================================

// -----------------------------
// CONFIG: choose ticker
// -----------------------------
$ticker = 'TSLA'; // default
// $ticker = isset($_GET['symbol']) ? $_GET['symbol'] : 'GOOG';

// -----------------------------
// INIT CNGN OBJECT (bitcoin engine)
// -----------------------------
$next = new CNGN(5); // 5 = some internal param from original class

// -----------------------------
// FILE PATHS
// -----------------------------
$dir = './tickers/';
$file_path = $dir . $ticker . '.json';

// -----------------------------
// FETCH DATA IF MISSING / OLD
// -----------------------------
if (!file_exists($file_path) || filesize($file_path) < 100000 || filemtime($file_path) < time() - 60*60*24*5) {

    // remove old file if exists
    if (file_exists($file_path)) unlink($file_path);

    // date range: 1 year ago → today
    $time_past = strtotime("-1 year");
    $time_now  = time();
    ...
    // Yahoo Finance CSV URL (1-day interval)
    $url = "https://query1.finance.yahoo.com/v7/finance/download/$ticker"
         . "?period1=$from&period2=$to&interval=1d&events=history&includeAdjustedClose=true";

    // fetch CSV via curl
    exec("curl --silent --location '$url' -o $file_path");
...
// RUN BITCOIN() LOGIC
// -----------------------------
$rets_sofar = $next->bitcoin($file_path, 15); // 15 = window size

// -----------------------------
// DISPLAY RESULTS
// -----------------------------
echo "<b style='margin-top:10px;margin-left:85%;position:absolute;float:right'>"
    . $rets_sofar[2] . "</b>";

echo "<br><br><br>" . $rets_sofar[1] . "% accuracy";

echo "<table style='width:100%;z-index:-1'>";
echo $rets_sofar[0]; // table or HTML content generated by bitcoin()
echo "</table>";
?>
