...
//      touch('./tickers/'.$ticker.'.json');
        ...
        {  
                $time_past = strtotime("now") - (60*60*24*365*17);
                $time_now = gmdate("Y-m-d", $time_past);
                unlink('./tickers/'.$ticker.'.json');
                exec("curl --location --request GET 'https://api.markets.sh/api/v1/symbols/NASDAQ:$ticker/quotes?from=$time_now&to=$time_now&api_token=35985512aa777c93dc4f99d8df50d25c' -o ./tickers/$ticker.json");
                chown('./tickers/'.$ticker.'.json','www-data');
                chgrp('./tickers/'.$ticker.'.json','www-data');
                chmod('./tickers/'.$ticker.'.json',777);
        }
        ...
        $rets_today = $next->bitcoin("./tickers/$ticker-today.json", 15);
